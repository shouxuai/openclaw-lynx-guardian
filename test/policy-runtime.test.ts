...
describe("policy runtime", () => {
  ...
  it("builds a warn-worthy trace when evidence scoring overrides raw allow", () => {
    const assessment: RiskAssessment = {
      level: "L0",
      score: 0,
      modules: [],
    ...
    const result = evaluateGuardDecisionPolicy({
      ...
      evidenceBundle: {
        eventKind: "output",
        summary: "sensitive output chain",
        modules: [],
        evidenceItems: [
          { dimension: "chain", weight: 2, confidence: 1, reason: "sensitive scope entered" },
          { dimension: "taint", weight: 4, confidence: 1, reason: "tainted artifact involved" },
        ],
        chainProgress: { stage: "sensitive_scope_entered" },
        isAuditWhitelisted: false,
      },
    });
    ...
    expect(trace.shouldWarn).toBe(true);
    expect(trace.raw.block).toBe(false);
    expect(trace.final.decision).toBe("block");
    expect(trace.evidence?.items.map((item) => item.reason)).toContain("tainted artifact involved");
  });
