...
describe("plugin approval compatibility", () => {
  ...
  it("classifies runtime versions into legacy, modern, and unknown tiers", () => {
    ...
    expect(classifyPluginApprovalRuntime("unknown")).toEqual({
      runtimeVersion: "unknown",
      tier: "unknown",
    });
  });
  ...
  it("defaults unknown webchat runtimes to the modern native approval path", () => {
    expect(
      resolvePluginApprovalCompat({
        runtimeVersion: "unknown",
        currentChannelProfile: "webchat",
  ...
  it("fails closed for legacy runtimes without a feishu route", () => {
    expect(
      resolvePluginApprovalCompat({
        runtimeVersion: "2026.3.27",
        currentChannelProfile: "webchat",
