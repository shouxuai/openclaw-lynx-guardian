...
describe("lynx check skill docs", () => {
  ...
  it("keeps the legacy daily skill name as an explicit compatibility alias", () => {
    const raw = readFileSync(
      new URL("../skills/lynx-guardian-daily-lynx-check/SKILL.md", import.meta.url),
      "utf8",
    );

    expect(raw).toContain("deprecated compatibility alias");
    expect(raw).toContain("lynx-guardian-check-orchestrator");
    expect(raw).toContain("requestId");
  });
});
