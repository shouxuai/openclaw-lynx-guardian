...
- **OpenClaw 服务检测**：插件启动后自动扫描本机 `127.0.0.1:18789` 与局域网目标，默认使用 `DEFAULT_CANDIDATE_PORTS`，可切换为全端口扫描。
- **插件私有检测配置**：首次运行会在插件目录生成 `lynx-discovery.config.json`，后续通过该文件控制 `fullScan`，无需修改 `openclaw.json`。
