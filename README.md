...
- **OpenClaw 服务检测 (SX-openclaw-discovery)**：手动触发检测局域网内 OpenClaw 服务实例，支持 IP/端口扫描与指纹识别。触发方式：输入 `check`、`/check`、`lynx-check`、`/lynx-check`、`openclaw-check`、`/openclaw-check`，或使用自然语言如 `检测龙虾IP`、`扫描openclaw端口`、`scan openclaw port` 等。
...
3. **分级响应机制**
   ...
   - 🟠 **中危 (Level 2)**: 触发二次确认机制，用户输入确认短语后方可放行。
   - 🟡 **低危 (Level 1)**: 允许操作，但在上下文中注入安全提示与价值观引导。
4. **二次确认机制**

   - 对于风险等级达到 L2–L4 的操作，插件不会直接拦截，而是暂停并要求用户输入确认短语（默认：`确认放行本次操作`）方可放行。
   - 每次确认仅对当前操作有效（基于操作指纹），且有效期默认为 90 秒，超时后需重新确认。
   - 部分高危模块（如提示注入 M1、凭证窃取 M5、恶意代码 M6 等）属于硬拒绝，不提供确认放行选项；过度代理 (M3)、受保护文件访问 (M2) 等模块默认支持二次确认。
   - 可通过配置 `riskPolicy.confirmationPhrase`、`riskPolicy.allowOneTimeOverrideLevels`、`riskPolicy.overrideTtlSeconds` 自定义确认短语、可放行等级和超时时间。
5. **自主安全提升**
