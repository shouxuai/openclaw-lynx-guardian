...
describe("EventsPage", () => {
  ...
  it("sends filters, shows bounded audit columns, and opens event details", async () => {
    fetchMock
    ...
    await screen.findByText("完整脱敏摘要");
    ...
    expect(fetchMock.mock.calls[2]?.[0]).toBe("/lynx/events?limit=10&cursor=cursor-page-2");
    ...
    expect(fetchMock.mock.calls[3]?.[0]).toBe(
      "/lynx/events?q=exec&riskLevel=L3&enforcementAction=requireApproval&limit=10",
    );
  });
});
