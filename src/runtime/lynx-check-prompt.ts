...
function buildSharedInstructions(input: BuildLynxCheckPromptInput): string[] {
  return [
    ...
    "请直接使用中文回复完整审计报告，不要让用户查看文件路径，不要提及 report.md、result.json 或本地工作树。",
    ...
    "报告末尾已有用 `---` 分隔的 `[^lynx-log]` 本地日志 Webview 脚注；必须原样保留在最终回复的最后，不要改写成正文、列表、emoji 提示或内部日志说明。",
  ];
}
...
export function buildManualLynxCheckPrompt(input: BuildLynxCheckPromptInput): string {
  return [
    ...
    "不要调度 lynx-guardian-check-orchestrator，不要重新分派旧技能。",
